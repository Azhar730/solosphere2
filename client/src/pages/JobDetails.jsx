...
const JobDetails = () => {
    ...
    const handleFormSubmission = async e => {
        ...
        if (price < parseFloat(min_price) || price > parseFloat(max_price)) return toast.error('Price Should be Equal to Minimum & Under Maximum')
        const comment = form.comment.value;
        const email = user?.email
        const deadline = startDate
        const status = 'Pending'
        const bidData = {
            jobId, price, deadline, comment, job_title, category,
            email, status, buyer
        }
        try {
            const { data } = await axios.post(`${import.meta.env.VITE_API_URL}/bid`, bidData)
            if (data.acknowledged) {
                toast.success('Bid Post Successful')
                navigate('/my-bids')
            }
        } catch (error) {
