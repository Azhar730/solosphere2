...
async function run() {
    try {
        const db = client.db('SoloSphere2')
        ...
        //post a single bid data in db
        app.post('/bid', async (req, res) => {
            const bidData = req.body
            const result = await bidsCollection.insertOne(bidData)
            res.send(result)
        })
        //get all bid data from db
        app.get('/bid', async (req, res) => {
            const result = await bidsCollection.find().toArray()
            res.send(result)
        })
        //post a single job data in db
        ...
        // get my-bids from db for a specific user
        ...
        // get bid-requests from db for job owner
